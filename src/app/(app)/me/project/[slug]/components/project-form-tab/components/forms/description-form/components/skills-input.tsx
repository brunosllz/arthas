...
export function SkillsInput(props: SkillsInputProps) {
  ...
  const {
    field,
    formState: { errors, isSubmitting: descriptionFormIsSubmitting },
  } = useController({
    name: 'skills',
    control,
  })
  ...
  useEffect(() => {
    if (inputRef.current) {
      inputRef.current.setAttribute('id', 'skills')
    }
  }, [])
                ...
                <Badge key={skill} variant="secondary">
                  ...
                  {props.isProjectAuthorOrOwner && (
                    <button
                      ...
                      disabled={descriptionFormIsSubmitting}
        ...
        {open && hasSearchValue ? (
          <CommandList className="absolute top-0 z-10 mt-2 w-full rounded-md border bg-popover text-popover-foreground shadow-xl shadow-black/30 outline-none animate-in">
            <CommandEmpty className="px-2 py-3 text-sm">
              {selectedItems && selectedItems.includes(search) ? (
