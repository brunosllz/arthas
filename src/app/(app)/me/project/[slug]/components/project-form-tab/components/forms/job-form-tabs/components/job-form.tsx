...
export function JobForm({
  ...
  const {
    ...
    reset,
    formState: {
      isValid: jobFormIsValid,
      errors,
      isSubmitting: jobFormIsSubmitting,
    },
  } = form

  function handleNextStep(data: JobFormInput) {
    const projectJobs = useBoundStore.getState().editProjectFormSteps.job.roles
    ...
    useBoundStore.setState((state) => ({
      editProjectFormSteps: {
        ...
        job: {
          ...state.editProjectFormSteps.job,
          roles: state.editProjectFormSteps.job.roles && [
  ...
  const roleItems = editProjectFormSteps.job.roleItens.filter((role) => {
    const currentProjectJobTabIsSelected =
      editProjectFormSteps.job.currentProjectJobTab
    ...
    if (editProjectFormSteps.job.roles) {
      const roleisSelected = editProjectFormSteps.job.roles.find(
  ...
  useEffect(() => {
    useBoundStore.setState((state) => ({
      editProjectFormSteps: {
        ...
        job: {
          ...state.editProjectFormSteps.job,
          editProjectJobIsValid: jobFormIsValid,
        },
      },
    }))
  }, [jobFormIsValid])
